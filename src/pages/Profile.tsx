...
const Profile = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    bio: userDummyData.bio,
    location: userDummyData.location,
  });
  ...
  useEffect(() => {
    if (data && Object.keys(data).length > 0) {
      setUserData((prevData) => ({
        ...prevData,
        name: get(data, "user.first_name", "") + " " + get(data, "user.last_name", ""),
        email: get(data, "user.email", prevData.email),
        avatar: get(data, "profile_picture_url", prevData.avatar),
      }));
